...
int main()
{
    string prime="91872654",base,b,m;
    base=subtract(prime,"1762");
    m=subtract(prime,"1");
    int s=0;
    while(m.back()=='0' || m.back()=='2' || m.back()=='4' || m.back()=='6' || m.back()=='8')
    {
        m=divide(m);
        s++;
    }
    b=base;
    string exponent="2";
    while(compare(m,exponent))
    {
        base=multiply(base,b);
        base=modulo(base,prime);
        exponent=add(exponent,"1");
    }
    if(base=="1")
    {
        cout<<prime<<" is a strong probable prime";
        return 0;
    }
    for(int i=1;i<s;i++)
    {
